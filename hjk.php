...
$sql5 = "INSERT INTO so_huo (sname,cname,s_time,mark,create_time,b_type,s_sun,s_ss) VALUES ('".$sname."','".$c_name."','".$s_time."','".$mark."','".$time."',".$sid.",".$jk9.",".$yp.")";
     //echo $sql5;
     $tyu=mysql_query($sql5, $connection);   

$sql51 = "select * from so_huo where create_time='".$time."'";
     //echo $sql5;
     $ty12=mysql_query($sql51, $connection);   
 $df12=mysql_fetch_assoc($ty12);
$hid= $df12['id'];
$hnum="W".date("Ymd",$time).$hid;
$sql15 = "update  so_huo set order_num='".$hnum."' where id=".$hid;
     //echo $sql5;
...
for($i=0;$i<count($jk1);$i++){
    $bn=$jk09[$i]*$jk6[$i];
$sql5 = "INSERT INTO so_huo_de (eid,sid,item,sname,snumber,r_number,price,smark,s_sun,sname_id,unit,jgc) VALUES (".$id.",'".$jk1[$i]."','".$jk2[$i]."','".$jk3[$i]."','".$jk5[$i]."','".$jk09[$i]."','".$jk6[$i]."','".$jk8[$i]."','".$bn."','".$jk4[$i]."','".$jk21[$i]."','".$jk211[$i]."')";
     //echo $sql5;
...
//推送数据到oa
//
$sql31 = "SELECT * FROM so_huo WHERE create_time='".$time."';";
...
$json = '{"order_num":"'.$hnum.'","shang_dai":"'.$sdm.'","eid":"'.$eeid.'","create_time":"'.$create_time.'", "sname":"'.$sname.'","cname":"'.$cname.'","s_time":"'.$s_time.'","mark":"'.$mark.'","s_sun":"'.$s_sun.'","sitem":"'.$sitem.'","price":"'.$sprice.'","snumber":"'.$ssnumber.'","ss_sun":"'.$ss_sun.'","sname_id":"'.$ssname_id.'","rrmark":"'.$srrmark.'","jgc":"'.$jgc1.'","user":"root", "pass":"1234"}';
